            ...
            $('body').on('row:action', function(e, a){
                switch(a.action){
                    case 'status':
                        ...
                        break;
                    case 'edit':
                        $.admin.api.open('{{$dbTable->access->getLang('edit_menu')}} : ' + a.row.title, '{{$dbTable->access->path('edit')}}?mid=' + a.row.mid, function(index, layer){layer.iframe.$('form').submit();}, function(index, report){
                            if(report.status){
                                $.admin.layer.close(index);
                        ...
                        break;
                    case 'add':
                        $.admin.api.open('{{$dbTable->access->getLang('add_menu')}} : ' + a.row.title, '{{$dbTable->access->path('add')}}?upId=' + a.row.mid, function(index, layer){layer.iframe.$('form').submit();}, function(index, report){
                            if(report.status){
                                $.admin.layer.close(index);
                                $.admin.alert.success('{{$dbTable->access->getLang('add_menu_success')}}');
                                $.admin.table.method('refresh');
                        ...
                        break;
                    case 'del':
                        $.admin.layer.confirm('{{$dbTable->access->getLang('del_menu')}} : ' + a.row.title, function(index){
