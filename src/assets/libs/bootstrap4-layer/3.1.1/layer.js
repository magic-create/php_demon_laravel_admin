...
;!function(window, undefined){
    ...
    var isLayui = window.layui && layui.define, $, win, ready = {
        ...
        btn:['ok', 'cancel'], //@todo hack 修改默认内容
    ...
    var layer = {
        ...
        alert:function(content, options, yes){
            ...
            return layer.open($.extend({
                title:ready.config.lang.alert(),    //  @todo hack 增加语言包
                content:content,
                yes:yes
            }, type ? {} : options));
        },
        ...
        confirm:function(content, options, yes, cancel){
            ...
            return layer.open($.extend({
                title:ready.config.lang.confirm(),    //  @todo hack 增加语言包
                content:content,
                btn:ready.btn,
                yes:yes,
                btn2:cancel
        ...
        msg:function(content, options, end){ //最常用提示层 @todo hack 修改样式
            var type = typeof options === 'function', rskin = ready.config.skin;
            var skin = (rskin ? rskin + ' msg' : '') || 'layui-layer-msg';
    ...
    Class.pt.vessel = function(conType, callback){
        ...
        callback([
            ...
            //主体 @todo hack 增加icon标识
            '<div class="' + doms[0] + (' layui-layer-' + ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin || '') + ' ' + (config.type == 0 && config.icon !== -1 ? 'icon' : '') + '" id="' + doms[0] + times + '" type="' + ready.type[config.type] + '" times="' + times + '" showtime="' + config.time + '" conType="' + (conType ? 'object' : 'string') + '" style="z-index: ' + zIndex + '; width:' + config.area[0] + ';height:' + config.area[1] + (config.fixed ? '' : ';position:absolute;') + '">'
            ...
            + (config.btn ? function(){
                ...
                for(var i = 0, len = config.btn.length; i < len; i++){
                    button += '<button class="btn ' + doms[6] + '' + i + ' ' + (i == 0 ? 'btn-primary' : 'btn-secondary') + '">' + (config.btn[i]) + '</button>';//@todo hack 将a标签调整为button
                }
                return '<div class="' + doms[6] + ' modal-footer layui-layer-btn-' + (config.btnAlign || '') + '">' + button + '</div>';
            }() : '')
    ...
    Class.pt.creat = function(){
        ...
        that.vessel(conType, function(html, titleHTML, moveElem){
            ...
            //  @todo hack 锁定焦点
            body.find('.layui-layer-btn0').focus();
        }).auto(times);
        ...
        //为兼容jQuery3.0的css动画影响元素尺寸计算 @todo hack 不移除css
    ...
    layer.style = function(index, options, limit){
        ...
        layero.css(options);
        btnHeight = layero.find('.' + doms[6]).outerHeight() || 0;//  @todo hack 增加判断
    ...
    layer.prompt = function(options, yes){
        ...
        //  @todo hack 增加内容区域
        if(options.content)
            options.content = content = '<h6>' + options.content + '</h6>' + content;

        var success = options.success;
        delete options.success;
        return layer.open($.extend({
            type:1
            , btn:[ready.btn[0], ready.btn[1]]
            , title:ready.config.lang.prompt()   //  @todo hack 增加语言包
            , content:content
            , skin:'layui-layer-prompt' + skin('prompt') + ' ' + layer.cache.skin //  @todo hack 主题皮肤
            , maxWidth:win.width()
            ...
            , yes:function(index){
                ...
                if(value === ''){
                    ...
                    //  @todo hack 增加语言包
                    layer.tips(ready.config.lang.maxlength(options.maxlength || 500), prompt, {tips:1});
                }else{
    ...
    layer.tab = function(options){
        ...
        return layer.open($.extend({
            type:1,
            skin:'layui-layer-tab' + skin('tab') + ' ' + layer.cache.skin, //  @todo hack 主题皮肤
            resize:false,
            title:function(){
    ...
    layer.photos = function(options, loop, key){
        ...
        }else if(data.length === 0){//  @todo hack 修改语言包
            return layer.msg(ready.config.lang.nopic());
        }
